...
public final class LambdaMetafactoryBootstrap {
    ...
    private LambdaMetafactoryPlan parseAltMetafactory(Object[] bootstrapArguments) {
        if (bootstrapArguments.length < 4 || !(bootstrapArguments[1] instanceof Handle implementationHandle)) {
            return new LambdaMetafactoryPlan(true, false, Optional.empty(), "altMetafactory missing implementation MethodHandle");
        }
        if (!(bootstrapArguments[3] instanceof Integer flags)) {
            return new LambdaMetafactoryPlan(true, false, Optional.empty(), "altMetafactory flags are not constant");
        }
        int index = 4;
        ArrayList<String> markers = new ArrayList<>();
        if ((flags & FLAG_MARKERS) != 0) {
            if (index >= bootstrapArguments.length || !(bootstrapArguments[index++] instanceof Integer markerCount)) {
                return new LambdaMetafactoryPlan(true, false, Optional.empty(), "altMetafactory marker count missing");
            }
            for (int markerIndex = 0; markerIndex < markerCount; markerIndex++) {
                if (index >= bootstrapArguments.length || !(bootstrapArguments[index++] instanceof Type markerType)) {
                    return new LambdaMetafactoryPlan(true, false, Optional.empty(), "altMetafactory marker type missing");
                }
        ...
        if ((flags & FLAG_BRIDGES) != 0) {
            if (index >= bootstrapArguments.length || !(bootstrapArguments[index++] instanceof Integer bridgeCount)) {
                return new LambdaMetafactoryPlan(true, false, Optional.empty(), "altMetafactory bridge count missing");
            }
            for (int bridgeIndex = 0; bridgeIndex < bridgeCount; bridgeIndex++) {
                if (index >= bootstrapArguments.length || !(bootstrapArguments[index++] instanceof Type bridgeType)) {
                    return new LambdaMetafactoryPlan(true, false, Optional.empty(), "altMetafactory bridge method type missing");
                }
        ...
        if ((flags & ~supportedFlags) != 0) {
            return new LambdaMetafactoryPlan(true, false, Optional.empty(), "unsupported altMetafactory flags " + flags);
        }
        return new LambdaMetafactoryPlan(
                true,
                true,
