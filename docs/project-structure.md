...
- `NativeFieldInternalizationPlanner` / `NativeFieldInternalizationPlan` / `NativeFieldStorageKind` / `NativeFieldInternalizationStorage`：produce immutable `INTERNALIZED` / `KEPT` decisions, distinguish mutable native slots from compile-time constants, retain exact descriptor storage kinds, deterministic per-owner reference indices, opaque slots and stable rejection reasons.
- `NativeFieldConstant`：保存仅供compiler消费的typed classfile constant；报告只写storage mode/hash evidence，不序列化原值。
- `analysis.method.NativeMethodUseAnalyzer` / `NativeMethodInternalizationPlanner` / `NativeMethodInternalizationPlan`：为private/protected static与same-owner exact instance method建立stable reverse-use/Handle/bootstrap/reflection evidence；对exact allowlisted public static应用declared-closed或current-JAR scope，对public instance消费完整input+classPath observer hierarchy/call facts，并输出immutable `INTERNALIZED` / `KEPT` decisions；不修改IR、registration或classfile。

Boundaries:

- The v1 planner accepts either an explicit `CLOSED_WORLD` assertion with a parse-complete supplied classpath, or a build-time, feature-scoped user approval for current-input-JAR-only analysis. The latter never changes `worldModel`, never parses configured classpath entries, and records that JAR-external callers/subclasses/reflection/JNI/agents/generated code are outside the accepted scope.
- public必须由required `publicMethodInternalizationAllowList`中的无wildcard exact selector显式授权。public static可使用declared `CLOSED_WORLD`或本次current-JAR-only Y授权；public instance只接受declared `CLOSED_WORLD`与parse-complete input+configured-classpath world。public instance不要求method/class为final，也不因可覆写slot本身产生`KEPT`，但所有调用点必须exact且caller仍须same-owner；实际non-exact dispatch会拒绝。已解析的exact reflection/MethodHandle/Handle/bootstrap/ConstantDynamic/EnclosingMethod observer或已知external Java entry产生`KEPT`；无法穷举的reflection/JNI/agent动态观察面进入warning/report，作为exact allowlist与world授权接受的风险。
- Current approval is narrower than a general field escape analysis. Mutable candidates are input-base `private static` primitive/reference/array fields with same-owner static or instance access methods; every observed accessor must finish as `nativeLowered` on an `LLVM_NATIVE_PATH` that supports the internalized storage ABI. Instance wrappers pass the field's declared defining `jclass`, never the receiver runtime class, to preserve one static state per defining Class. A separate `private static final ConstantValue` path folds exact same-owner primitive reads to SSA constants without runtime storage and may remove primitive/String declarations already proven to have no field references; explicit String reads stay in the JVM to preserve intern/object identity. Any cross-owner, unselected, `skipped`, non-LLVM or dynamic observer keeps the field in the JVM. Instance fields and volatile/unsupported-metadata/`<clinit>` shapes also remain JVM fields.
- Any unresolved field reference or dynamic observation surface rejects candidates conservatively.
- IR mutation belongs to `ir.pass.protection.NativeFieldIrRewriter` / `NativeConstantFieldIrFolder`; final-plan validation belongs to `pipeline.FieldInternalizationFinalPlanValidator`; classfile removal and residual-reference audit belong to `packaging`.
...
- `JdkPureNativeIntrinsicPass`在ordinary optimization后、protection前处理exact same-block/unique-use JDK调用组合；首个shape是`ByteBuffer.allocate(4).putInt(i).array()`。它只改IR并登记helper，不生成C/JNI源码。
...
- `methodInternalization`同样默认关闭；required exact `publicMethodInternalizationAllowList`默认空。use/observer analysis位于`analysis.method`，public declared/current-JAR scope policy与final-plan strategy/validation位于`pipeline`，nested-local-frame internal dispatch位于`toolchain`，MethodNode removal/residual audit位于`packaging`；各层共用同一immutable logical plan。随后`NativeOnlyMethodCoalescingPlanner`/`Coordinator`形成独立physical-retention plan，LLVM compiler、C binding filter与artifact audit共同验证standalone callee surface消失。
...
- attacker-audit metrics当前是独立回归输入/报告合同，不修改compiler artifact，
  也不重新推断protection pass applicability。此前carrier reuse/size接口已用于
  ...
  回归，也不覆盖当前semantic-surface强制profile、`ReleaseSmall`与shared cleanup
  变更；后者必须另行采集dual-build和final-binary evidence。
...
- `toolchain.NativeLocalAbiProfile` / `NativeLocalAbiPlanner` /
  `HostNativeLocalAbiBridgeSource`：按final native ABI选择bounded profile。
  传递`JNIEnv*`或owner `jclass`的JVM/JNI semantic-surface binding强制使用
  branched参数重排；pure-native scalar binding仍从direct canonical、单层、双层与
  branched四种shape中派生build-scoped topology。branched通过wrapper
  activation-local volatile predicate在一层与两层route间选择，最多生成三个
  `static __attribute__((noinline, used))` bridge且不使用`optnone`；不得生成持久
...
- packaging 只重写 final status 为 `nativeLowered` 的 method；对普通Java入口使用 `nativeOriginal`，对 `<init>`、`<clinit>` 和有 Code 的 interface method 使用合法 stub/helper strategy；对final-plan批准的`internalNativeOnly`精确删除整个MethodNode且不注入额外registration。是否保留独立hidden body或采用`coalescedNativeOnly`只由上游physical-retention plan决定，packaging不重新推断。
...
- packaging 的 method transform 只消费 final validated method plan；删除必须原子，写JAR后阻断批准method的declaration、MethodInsn、LDC Handle、invokedynamic/ConstantDynamic bootstrap和EnclosingMethod residual。
...
- `ZigCInputMachinePolicyPlan`
- `ZigCSourceMachinePolicyManifest`
...
- `NativeRegistrationControlTopologyPlanner`
- `NativeRegistrationControlTopologyPlan`
- `NativeRegistrationControlSymbolDeriver`
- `HostNativeRegistrationChunkSource`
- `NativeRegistrationControlSourceVerifier`
- `NativeRegistrationAggregateSourceVerifier`
- `NativeRegistrationControlPreprocessorVerifier`
- `NativeRegistrationControlLexicalScanner`
- `NativeRegistrationResolverPlan`
- `HostNativeRegistrationResolverSource`
...
- Zig archive download/extraction：`ZigDownloader` / `ZigArchiveExtractor`，先使用 `<j2ll-home>` 已存在 archive，没有才下载；`ZigArchiveVerifier` 必须在解压前校验 local/downloaded archive SHA-256，失败作为 native/toolchain error。`ManagedZigLocator`以normalized home为粒度同时持有JVM内锁与filesystem lock；download只写唯一temporary file，校验后才rename发布，extract也先进入唯一temporary directory并在完整成功后发布到`<j2ll-home>/zig`，并发Gradle/process不得观察或覆盖半成品。默认Dummy E2E在没有显式`j2ll.realHome`/`J2LL_REAL_HOME`时才按需使用`<GRADLE_USER_HOME>/caches/j2ll/zig/<version>/<host-archive>/`；Gradle `--offline`只允许已有install/archive，且该test-only home不得作为全局real-Zig opt-in传播给其他suite。
- Zig build manifest/source generation：`ZigBuildWriter`，为 selected target matrix 生成一个 `build.zig` 和一个 stable manifest；generated C compile unit使用`ReleaseSmall`，per-class LLVM input与final link module保持`ReleaseSafe`。`ZigBuildProgressPlan`将最多64个observable compile unit按source kind同质分组，避免C与LLVM混用optimization mode。每个C input由`ZigCAssemblyCompileEmitter`通过managed Zig `cc -Oz -S`只编译一次到workspace-private `native/zig-workspace/evidence/optimized-assembly/<target>/<c-id>.s`；同一assembly `LazyPath`同时交给machine gate和`addAssemblyFile`实际链接，禁止独立二次C compile或把不覆盖pure-C input的`getEmittedAsm()`当证据。evidence install是compile marker的依赖，但不进入final JAR或report。`build.zig` 只为当前 preflight 判定 buildable 的 target 生成 install artifact，manifest/report 仍必须列出全部 selected/required target。当前 preflight 无法构建的 required target 进入 `failedTargets`，reason 使用 `ZIG_TARGET_UNBUILDABLE`，并使 pipeline failed。
...
- generated/native machine policy：`ZigTargetBuildPolicy`组合`NativeUnwindRetentionPolicy`和`NativeMachineOutlinerPolicy`。Windows因SEH强制保留unwind并默认禁用machine outliner；`--debug`与config requested retain也强制保留。Linux/macOS requested=false时，generated-C unit使用no-unwind flags；`NativeLlvmSourcePlan`只为final canonical module proof为`PROVEN_ABSENT`的source选择结构化`nounwind`变体，`REQUIRED`/`UNKNOWN`或unmodeled object input保留。C flags和Zig module option都不得被当作会改写`addObjectFile` `.ll`的证据。`ZigCInputMachinePolicyPlan`对完整C source set形成total immutable mapping，并把`JniWrapperEmission.path()`按规范化exact Path唯一绑定为registration-control input；该input六目标显式`outliner=never`，其他runtime C在Linux/macOS继续使用16-byte阈值、Windows继续默认关闭。manifest保留target-default兼容摘要，并新增逐target/per-input mode、effective `machineOutlinerCFlags`/reason与assembly evidence；build graph与manifest消费同一compile-input inventory，不得从source文件名或`c-0`下标重建决策。
...
- registration C generation：`HostNativeRegistrationSource` consumes either the ordinary registration plan or the exact `MethodTableHidingPlan`; transient physical layouts are never reconstructed from a boolean inside the emitter. `NativeRegistrationResolverPlan` / `HostNativeRegistrationResolverSource` encode the unique Loader anchor separately, reserve bounded local-reference capacity and create one activation-local resolver context. Only that anchor uses `FindClass`; business owners use its exact defining loader with non-initializing `Class.forName(..., false, loader)`, with no TCCL/system fallback or global class cache. `NativeRegistrationTextPlan` / `NativeRegistrationTextStorageLayout` reuse equal method-name/descriptor text only inside one owner and purpose domain, then form groups of at most 8 values and 512 decoded bytes; one oversize value owns one group, and no group crosses owner/purpose. `NativeRegistrationStoragePlan` selects bounded stack storage only for at most 64 bindings and at most 16 KiB total decoded text; larger owners retain heap allocation. Both paths zero text scratch and `JNINativeMethod[]`, and the heap path then frees them. `HostNativeRegistrationFailureLeafSource` owns four registration-domain hash-only `noinline,cold` failure leaves whose only argument is `JNIEnv*`; aggregate cleanup/rollback uses activation-local `jclass[]` plus `registered_count` instead of per-owner duplicated root control flow。`NativeRegistrationControlTopologyPlanner` / immutable `NativeRegistrationControlTopologyPlan`先冻结完整control symbol set和build-derived physical owner order；nonzero-owner另冻结exact三个hash-only entry route与两条路径`root -> R0 -> aggregate` / `root -> R1 -> R2 -> aggregate`，再形成最多8个连续balanced forward chunk。`NativeRegistrationControlSymbolDeriver`只发出exact `[a-p]{32}`，plan在完整集合上拒绝与registration material碰撞。`HostNativeRegistrationRootSource`保持resolver/owner array/count/success cleanup/rollback/Throwable restore只位于唯一aggregate activation；`HostNativeRegistrationRouteSource`只转发activation参数并发出direct call与volatile continuation。`HostNativeRegistrationChunkSource`只materialize owner call、success count更新、唯一next direct call与post-call continuation，不拥有resolver/JNI exception/rollback。`NativeRegistrationControlCFunctionPolicy`通过attributed prototype统一给root/route/aggregate/chunk/owner施加`noinline,disable_tail_calls`；三个route使用exact三种typed recipe，最多八个chunk使用无重复的独立八种closed structural variant。`NativeRegistrationControlSourceVerifier`在registration fragment和最终完整TU各fail closed一次；它把failure/owner/route/chunk closure委托给小型schema verifier，把唯一aggregate exact tail委托给`NativeRegistrationAggregateSourceVerifier`，并通过一次`NativeRegistrationControlLexicalScanner`索引与`NativeRegistrationControlPreprocessorVerifier`排除comment/string/inactive-preprocessor证据。新增结构预算为nonzero-owner exact三个route definitions加`min(8, ceil(ownerCount / 4))`个chunk definitions（zero owner两者均为0），没有table、padding、dispatcher或持久data；optimized compile-unit assembly gate另阻断tail collapse，在exact wrapper的全部code surface拒绝machine-outliner artifact，并要求volatile same-slot readback前stack/frame base保持稳定。
- LLVM JNI proxy relocation：planner在coalescing后冻结method-to-entry plan并复用原
  ...
  三个bridge和独立NOINLINE semantic body。`NativeJniProxyAbiProjection`只允许ordinary
  `NATIVE_ORIGINAL`的`V/I/J/F/D/L/[` physical参数投影；static可映射env/owner，instance
  可映射env/self但不能额外合成defining-owner class。Reference/array handle按`ptr`原样转发。
  semantic env/owner/reference/local-reference/exception/runtime-metadata surface强制branched
  topology，proxy/bridge本身不得执行JNI、异常或local-ref操作。Compiler structured-model
  gate验证exact physical/semantic ABI、ordered arguments、topology caller closure与closed CFG；
  generated-C gate验证只保留exact proxy
...
- native text：`toolchain.nativetext.NativeTextEncoder` 生成 build/purpose/use-scoped
  ...
  identity是明确窄例外。`NativeTextCipherReferenceAudit`先对整份generated C执行一次
  lexical mask和通用reference分类索引，再按cipher读取局部facts；不得恢复逐cipher
  重新mask/正则扫描整份translation unit的乘法复杂度。
