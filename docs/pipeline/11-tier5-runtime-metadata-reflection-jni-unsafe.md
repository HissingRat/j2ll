...
- Release-readiness support facts are also summarized in `reports/support-matrix.json`, including Tier 5 rows for static reflection helper coverage and raw Unsafe、dynamic VarHandle、MethodHandle adapter、unsupported finally 的 skipped reasons。Strict release readiness requires blocker reasons to map to release suite cases or weird-bytecode seeds。Structural cross-build success for all six fixed targets is separate from the still-pending non-host runtime E2E evidence。
...
- 简单 catch-all rethrow/finally cleanup 已 lower；multi-exit finally、exception state merge、monitor/finally interaction 等仍是 `skipped`，使用精确 reason code。
