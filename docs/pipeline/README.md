...
- `fieldInternalization` 的字段候选仍只限`private static`，但same-owner static或instance accessor都可批准；每个真实accessor必须最终为`nativeLowered`并走合格的`LLVM_NATIVE_PATH` storage ABI。instance wrapper使用field的declared defining `jclass`访问sidecar。任一 accessor 为 `skipped`、unselected、cross-owner、non-LLVM或存在动态观察边界时保留 JVM field；reference/array 状态仍由 Loader 的 JVM-managed `ClassValue<Object[]>` sidecar持有。
