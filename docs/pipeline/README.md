...
- `nativeLowered`：原业务语义由已验证的 LLVM、生成式 template/stub 或 JNI/runtime helper-backed native implementation 完成。普通Java入口具有Code rewrite与`RegisterNatives`证据；批准为`internalNativeOnly`的方法具有native-caller closure，output class不再含method_info且不注册。其物理实现可以保留独立hidden LLVM body，也可以在严格single-call-site证明后只合并存在于caller body中。
...
- 主线的selected-method lowering由独立`SelectedMethodLoweringCoordinator`交付CFG、raw SSA、optimized IR与initializer plan；final LLVM pass evidence由`NativeLlvmProtectionEvidenceCoordinator`汇总。`MainlinePipeline`只编排这些immutable artifacts和diagnostic传播，不内嵌method lowering或逐module report重建逻辑。
...
- `fieldInternalization` 的字段候选仍只限`private static`。可变字段的same-owner static或instance accessor都可批准；每个真实accessor必须最终为`nativeLowered`并走合格的`LLVM_NATIVE_PATH` storage ABI。instance wrapper使用field的declared defining `jclass`访问sidecar，reference/array状态由Loader的JVM-managed `ClassValue<Object[]>` sidecar持有。另一个无runtime-storage路径会把same-owner LLVM-native显式读取的primitive `ConstantValue`折叠为SSA常量，并删除已经没有任何field reference的primitive/String常量声明；显式String `GETSTATIC`因intern/object identity语义继续保留。任一访问为`skipped`、unselected、cross-owner、non-LLVM或存在动态观察边界时保留JVM field。
- `methodInternalization`在final LLVM plan后运行，通常批准private/protected static或same-owner exact private/protected instance method。required exact `publicMethodInternalizationAllowList`可另行授权public static及same-owner exact public instance：public static可使用declared `CLOSED_WORLD`或本次Y授权的current-JAR-only scope；public instance只接受declared `CLOSED_WORLD`及parse-complete input+classPath world，不要求method/class为final，也不因可覆写slot本身拒绝，但每个调用点必须exact且caller仍须same-owner。已解析的exact reflection/MethodHandle/Handle/bootstrap/ConstantDynamic/EnclosingMethod observer或已知external Java entry会保留入口；无法穷举的reflection/JNI/agent动态观察面作为allowlist/world授权接受的风险进入warning/report。批准项仍为`nativeLowered`，但Java declaration与`RegisterNatives` binding被移除；cross-owner static和reference-returning caller通过nested-local-frame internal dispatch bridge进入hidden native wrapper，same-owner direct scalar路径可继续使用LLVM direct ABI。随后独立coalescing stage只把恰有一个直接native call site、pure scalar/non-throwing且没有field/call/monitor/JNI-owned-reference语义的internal-only小方法合并进caller；证明不全时保留原hidden body。
...
- artifact audit 验证每个 `nativeLowered` method 的native artifact closure：registered入口有唯一physical entry与registration；`generatedCWrapper`必须有C wrapper，`llvmJniProxy`必须有独立hash-only proxy、原ABI semantic body、完整bounded local-ABI topology和exact structured call closure，且generated C中没有logical wrapper/body/bridge残留。`internalNativeOnly`入口没有Java declaration/registration且所有MethodInsn/Handle/bootstrap/EnclosingMethod residual为零；`coalescedNativeOnly`还必须没有callee LLVM function/declaration/reference、generated-C wrapper或workspace source symbol残留。每个 `skipped` method 的原 body 保留且无 registration，fallback bytecode复制表面在 generated C/native/JAR 中不存在。
