...
Current implementation records `preservationSummary` in `reports/packaging-report.json` with manifest, service entry, module-info, multi-release and versioned-entry facts. Versioned classes are preserved as entries, while native lowering still targets base class entries only (`baseClassesOnlyPreserveVersionedEntries`). A child JVM multi-release/service fixture verifies that a selected base class rewrite does not corrupt a different versioned class selected by the runtime.
