...
- JNI helper产生的owned local reference只会在registered native method返回时由JVM自动回收，helper返回、internal LLVM callee返回和loop iteration本身不会释放它。后端只有在能证明reference origin/last use并发出正确`DeleteLocalRef`时才可让该值在native cycle中反复产生。当前 planner 对 frozen same-owner direct-call closure传播 reachable owned-ref production 摘要（包括void/primitive callee内部产生的reference），并检查method CFG cycle、循环内callee、传递调用链和direct-call SCC；不能证明有界的受影响方法由final native coverage以`UNBOUNDED_JNI_LOCAL_REFERENCE_LIFETIME`整方法跳过。
