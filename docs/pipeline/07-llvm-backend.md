...
当前 backend 还没有把 JNI/runtime helper 产生的 pending exception 物化为 native method 内部的 exception CFG。因此，当 call、allocation、field/class-init、array/type 或其他可抛 JVM exception 的 instruction 位于 user try/catch region 时，planner 必须拒绝 `LLVM_NATIVE_PATH`，把完整 method 记录为 `skipped` + `UNSUPPORTED_PROTECTED_JVM_EXCEPTION_FLOW`，保留原 Code 且不生成 registration；这与仅用于 `ACC_SYNCHRONIZED` unwind 的 `$sync_cleanup`、仅用于 `<clinit>` unwind 的 `$class_init_failed` synthetic edge 区分。无 user handler 的 nested reference-array allocation（例如 `ANEWARRAY [B` 产生 `byte[][]`）可使用 `FindClass("[B")` + `NewObjectArray` 的 JNI allocation path。
