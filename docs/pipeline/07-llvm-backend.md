...
- `NativeLlvmCompiler` 是 final LLVM compilation 的唯一生产者：它只接收 final `LLVM_NATIVE_PATH` implementation 与其可达 compiler-internal helper，并把同一组 module/pass result 同时交给 protection report、intermediate dump 和 Zig source writer。template/fallback method 不得另行 lower 后制造虚假 `RAN` 或只存在于报告中的 symbol。
...
Block parameters lower 成 LLVM `phi`。`THROW` terminator 生成 env-backed `j2ll_rt_throw(ptr %j2ll_env, ptr throwable)`，随后返回 descriptor 对应的 pending-exception-safe placeholder；native wrapper 返回 JVM 后，pending exception 可由 native 方法的 Java caller 处理。Integer div/rem 和 array/type/allocation/reflection helpers 使用同样的 pending exception convention。volatile/final/monitor JMM marker 生成 conservative LLVM `fence`；Thread start/join happens-before marker 生成固定 runtime helper call。

当前 backend 还没有把 JNI/runtime helper 产生的 pending exception 物化为 native method 内部的 exception CFG。因此，当 call、allocation、field/class-init、array/type 或其他可抛 JVM exception 的 instruction 位于 user try/catch region 时，planner 必须拒绝 `LLVM_NATIVE_PATH`。ordinary method 改走 encoded `nativeEmbeddedClassBlob`，记录 `halfLowered` + `UNSUPPORTED_PROTECTED_JVM_EXCEPTION_FLOW`；这与仅用于 `ACC_SYNCHRONIZED` unwind 的 `$sync_cleanup`、仅用于 `<clinit>` unwind 的 `$class_init_failed` synthetic edge 区分。无 user handler 的 nested reference-array allocation（例如 `ANEWARRAY [B` 产生 `byte[][]`）可使用 `FindClass("[B")` + `NewObjectArray` 的 JNI allocation path。
