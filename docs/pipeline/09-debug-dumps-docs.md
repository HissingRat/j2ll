...
- useful failure diagnostics for expected failure cases such as invalid config, artifact audit failure, signed input rejected by `signaturePolicy: "fail"` or required non-host targets rejected with `ZIG_TARGET_UNBUILDABLE`; failed config/pipeline runs also write `failure-report.json` with `finalArtifactWritten=false`.
