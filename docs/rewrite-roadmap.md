...
- 原 implementation checklist 的 8 项已按职责进入 program/IR、LLVM model、packaging/native registration 和 field-analysis pipeline，并已有通过的 Windows real-Zig host child-JVM 与六目标 feature-specific structural evidence；新增 method internalization 已进入 final-plan/packaging pipeline，先由 focused 与 v2 sample evidence验收。后续按同一 checklist 收口更广的 host boundary、optimized machine-code retention 和 non-host runtime evidence，不把 cross-link 扩大成目标 OS/JVM 已运行。
...
- 已接实 IR `StringEncryptionPass`、`PrimitiveConstantEncryptionPass`、`ControlFlowFlatteningPass`、独立 `BasicBlockSplittingPass`、独立 `FakeBranchesPass` 和 `BlockNameObfuscationPass`，并在 all-on JVM-hosted E2E 中验证不破坏现有 native lowering vertical slice。`fakeBranches` 与 `basicBlockSplitting` 是不同的必填 boolean，`blockNameObfuscation` 是新增必填 IR boolean；三者产生独立 pass/report row。`FakeBranchesPass` 会真实改写 protected IR，但当 method 没有可用动态参数而使用常量 predicate fallback 时，managed Zig `ReleaseSafe` 可能将该 branch 优化消除，不能宣称最终 binary 一定保留。`PrimitiveConstantEncryptionPass` 现在覆盖 `int` / `long` XOR split，以及 `float` / `double` raw-bit XOR + LLVM bitcast，包含 `NaN`、`-0.0` 和 infinity 的 child JVM raw-bit parity。`ControlFlowFlatteningPass`已从整method shape gate扩展为bounded single-entry/multi-exit region plan：每个method最多4个互不重叠region、每个region最多32个原始member block；owned/exception/handler/monitor/JMM/class-init敏感block留在region外，内部edge才走region-local dispatcher，exit直接保留原外部target。每个region的state/default target由per-build/per-method/per-region材料派生，state固定为`[0, regionMemberCount)`的dense permutation，不引入额外table或查表transition。至少一个region通过validator才报告affected；region-level固定样本覆盖率要等真实v2重跑后记录，不能沿用或预估历史数字。
...
- `fieldInternalization` 已与真实实现同步进入 schema，默认 `false`，在 declared `CLOSED_WORLD`，或 build-time Y 明确授权的 current-JAR-only scope 下处理 input-base `private static` `boolean/byte/short/char/int/long/float/double` 与 JVM reference/array、same-owner static或instance accessor，且所有accessor的final implementation path都必须是支持对应storage ABI的`LLVM_NATIVE_PATH`。current-JAR-only 是 feature-scoped invocation decision，不改写 `worldModel`、不读取配置 classpath，N/EOF fail closed，validate/dry-run 只报告待确认 warning。可变primitive使用per-defining-`jclass` weak-keyed relaxed atomic raw-bit state，窄整数遵循JVM descriptor截断/扩展，float/double通过LLVM bitcast保留raw bits；reference/array值由唯一`Loader.class`的per-defining-Class `ClassValue<Object[]>` sidecar在JVM heap强持有。另一个compile-time路径把获准`private static final` primitive `ConstantValue`的显式same-owner读取在protection前折叠为SSA常量，不分配slot/sidecar；零field-reference primitive/String declaration可删除，显式String读取继续保留以维护intern/object identity。instance accessor wrapper传递field的declared defining `jclass`，不得按receiver runtime class分裂static storage。final-plan validation、FieldNode removal 和 residual-reference audit 共用同一批准计划；gated host E2E 已覆盖全部可变类型边界值、NaN payload/negative zero、Object identity/null/GC strong hold、instance `byte[]` accessor的基类/子类receiver共享、并发与双 ClassLoader隔离，六目标专项已覆盖真实 slot call/storage/sidecar build-graph、privacy 与 export，non-host runtime 与 deterministic real-Zig dual-run仍待补。
- `methodInternalization` 已进入 schema且默认`false`，使用独立whole-program confirmation requirement。final-plan planner批准ordinary `LLVM_NATIVE_PATH`的private/protected static（允许native caller cross-owner）和same-owner exact private/protected instance method；required `publicMethodInternalizationAllowList`以无wildcard、无重复的exact method selector另行授权public static及same-owner exact public instance。public static可使用declared `CLOSED_WORLD`或本次current-JAR-only Y授权；public instance只接受declared `CLOSED_WORLD`和parse-complete input+configured-classpath world，不要求method/class为final，也不因可覆写slot本身拒绝，但所有调用点必须exact且caller仍须same-owner。已解析的exact reflection/MethodHandle/Handle/bootstrap/ConstantDynamic/EnclosingMethod observer、已知external Java entry、unselected/skipped/template caller、non-exact dispatch、MR owner或unsupported method kind都会保留普通Java native declaration；无法穷举的reflection/JNI/agent动态观察面不再全局否决exact allowlisted public，而是作为用户接受风险进入warning/report。批准项仍为`nativeLowered`，Java `method_info`与`RegisterNatives` binding删除；默认retention为`internalNativeOnly` hidden LLVM body，随后严格unique-direct-call/pure-scalar/non-throwing coalescing可把其physical retention改为`coalescedNativeOnly`并完全消除callee独立LLVM/C symbol surface。cross-owner/reference-sensitive route通过defining-class + nested-local-frame bridge保留GC/pending-exception语义；logical internalization plan、physical coalescing plan、final validator、MethodNode removal和recursive residual audit分层共用。
- 2026-07-31的v2五目标证据保持71 `nativeLowered` / 0 `skipped`，其中protected static target已经验证`internalNativeOnly`删除与artifact residual audit；2026-08-01旧public fixture只证明final public instance子集的runtime语义。旧v2中“未解析reflection site全局保留7个allowlisted public static”的结果属于已被本策略取代的历史边界，不再作为当前验收。2026-08-01 02:52的新v2五目标构建仍为71 `nativeLowered` / 0 `skipped`，但达到14个`internalNativeOnly`与57个registration：4个有真实native caller closure的allowlisted `LaoShuUtils` public static已删除，另3个以`METHOD_INTERNALIZATION_NO_NATIVE_CALLER`保留；17个未解析reflection site只产生aggregate warning，artifact/symbol/plaintext/residual audit全部通过。focused与Windows real-Zig Dummy/runtime证据另覆盖resolved exact observer硬拒绝、动态observer warning、non-final same-owner exact public instance parity、non-exact/cross-owner拒绝、incomplete hierarchy与known callback保留。
...
- Generated C compile unit使用`ReleaseSmall`，per-class LLVM input与final link module保持`ReleaseSafe`；最多64个observable compile unit按source kind同质分组，确保batch不会让C与LLVM共用错误optimization mode。2026-07-31 v2五目标实测中，native raw总量由3,502,027 B降到2,611,446 B（-25.43%），Windows x64由686,080 B降到503,296 B（-26.64%），output JAR由3,969,705 B降到3,872,737 B（-2.44%）；第二随机build的五库raw总量为2,616,590 B，各目标相对第一轮仅0%–0.86%波动。两轮均为71 `nativeLowered`、0 `skipped`且artifact/symbol audit通过。
...
- final method-internalization plan批准的ordinary method使用`internalNativeOnly`：删除完整method declaration与registration，同时让hidden implementation通过native caller closure保持可达。
...
- lowering/packaging report明确区分`registeredNative`、`internalNativeOnly`与`javaBytecode` retention；artifact audit阻断internal-only declaration/invocation/Handle/bootstrap/EnclosingMethod残留。
...
- method internalization通过`METHOD_INTERNALIZATION` protection coverage、lowering retention fields与packaging per-method presence fields记录；不新增第三种method outcome。artifact audit递归验证批准method的classfile residual为零，同时允许它不出现在registration groups。
