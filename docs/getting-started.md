...
`methodInternalization` uses a separate prompt with the same lifecycle. Y authorizes only current-input-JAR call/override/observer analysis for private/protected candidates and exact-allowlisted public static candidates; configured classpath, JAR-external callers/subclasses and external reflection/JNI/agent observers stay out of scope and are reported as accepted risk. Public instance methods still require configured `worldModel: "CLOSED_WORLD"` plus an exact, wildcard-free entry in `protection.ir.publicMethodInternalizationAllowList`. If both internalization features are enabled outside `CLOSED_WORLD`, answer both prompts in their displayed order.
...
An approved method-internalization candidate is still `nativeLowered`, but its Java declaration and `RegisterNatives` row are both absent. The current subset supports private/protected static methods, including protected static calls from another native-lowered owner, plus private/protected instance methods whose callers are all in the same owner and whose dispatch target is exact. Exact-authorized public static methods work in declared closed world or under the same current-JAR-only Y decision. Public instance methods require declared closed world and a same-owner caller closure whose every dispatch resolves exactly; the method/class need not be final, and a potentially overridable slot alone is not rejected. A resolved exact reflection/MethodHandle/Handle/bootstrap/ConstantDynamic/EnclosingMethod observer, launcher/agent entry, or closed-catalog JVM/JDK callback still fails closed. The callback catalog requires a real hierarchy relationship and exact descriptor for Object virtual methods, Runnable/Callable, Thread/TimerTask, serialization, Comparator, and common functional interfaces; it does not reject ordinary override slots by default. Third-party framework callbacks outside that closed catalog and unsupported or unbounded reflection/JNI/agent observation remain a user-accepted risk, never proof that no external observer exists.
