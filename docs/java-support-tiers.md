...
- Runtime Loader 始终包含 native loading，只有 implementation plan 实际使用 `nativeEmbeddedClassBlob` 时才保留 `defineHiddenFallback`；旧 `J2llFallbackSupport.class`、`J2llNativeLoaderSupport.class` 和 `j2ll/generated/**/NativeLoader.class` 不再输出。`embeddedLibraryDirectory` 必须是规范 Java internal package path，输入 base/MR 同名 Loader 在 Zig 前失败。同一 defining `ClassLoader` 中不同产物使用相同目录仍会得到同名 Loader，是明确已知边界；建议使用应用唯一目录，独立 ClassLoader 保持隔离。
